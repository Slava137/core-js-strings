...
function removeFirstOccurrences(str, value) {
  ...
  return indexValue !== -1 ? str.replace(value, '') : str;
}
...
function removeLastOccurrences(str, value) {
  const lastIndexValue = str.lastIndexOf(value);

  return lastIndexValue !== -1
    ? str.slice(0, lastIndexValue) + str.slice(lastIndexValue + value.length)
    : str;
}
...
function sumOfCodes(str) {
  return str
    ? str.split('').reduce((sum, item) => sum + item.charCodeAt(), 0)
    : 0;
}
...
function findLongestWord(sentence) {
  ...
  const sortArrSentences = arrSentences.sort((a, b) => b.length - a.length);

  return sortArrSentences[0];
}
...
function reverseWords(str) {
  ...
  const reverseArrStr = arrStr.map((item) => item.split('').reverse().join(''));
...
function invertCase(str) {
  ...
  const invertArrStr = arrStr.map((char) =>
    char === char.toLowerCase() ? char.toUpperCase() : char.toLowerCase()
  );

  return invertArrStr.join('');
}
